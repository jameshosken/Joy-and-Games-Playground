...
public class InputHandler : MonoBehaviour
{
    ...
    */
    ...
    void HandlePCControls()
    {
        ...
        {
            roll = Mathf.Clamp(roll + 0.05f, -1, 1);
        }
        ...
        float fwd = Input.GetKey(KeyCode.LeftShift) ? 5 : 0;
        float back = Input.GetKey(KeyCode.LeftControl) ? -3 : 0;

        shipControls.ApplyRelativeThrust(new Vector3(0, 0, fwd+back));


        if (Mathf.Abs( fwd+back) > 0.01f)
        {
            shipShake.SetIsShaking(true);
        }
        else
